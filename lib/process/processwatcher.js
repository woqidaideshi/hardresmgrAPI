 ...
 *    null
 */
ProcessInfo.prototype.startWatch = function(pid) {
    var self = this;
 ...
 *    null
 */
ProcessInfo.prototype.stopWatch = function(pid){
    var self = this;
    for (var i = 0; i < self.pidarray.length; i++) {
        ...
        if (self.pidarray[i].pid == pid) {
            ...
            for (var j = 0; j < tempcb.length; j++) {
                if (tempcb[j] == callback) {
                    ...
                    tempcb.splice(j, 1);
                    if (tempcb.length == 0) {
                        self.stopWatch(self.pidarray[i].interval);
                        emptyflag = i;
                    }
                    break;
                }
 ...
 *    null
 */
ProcessInfo.prototype.doAction = function(pid, usage) {
    var self = this;
    for (var i = 0; i < self.pidarray.length; i++) {
        if (self.pidarray[i].pid == pid) {
            var tempcb = self.pidarray[i].cb;
...
ProcessInfo.prototype.echo = function() {
    var self = this;
