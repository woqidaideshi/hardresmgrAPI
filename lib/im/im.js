...
var fileTransferServer = require("./imChat/fileTransferServer");
...
var dataTransferProxy = require('../interface/dataTransferProxy').getProxy();
    ...
    if (MsgObj.localUID === accSetItemKey) {
      ...
      if (accSetItem===undefined||!net.isIP(accSetItem.toIP)) {
        ...
        imChatPack.sendMSGbyUID(ipset, accSetItem.toAccount, MsgObj.Msg, MsgObj.App, function(recMsg) {
          var intent = new base.intent(MsgObj.App, recMsg);
          intent.send(MsgObj.App, MsgObj.IP);
          if (msgRst === undefined && msg.err === undefined)
            msgRst = recMsg;
          if ((++countFlag) === len) {
            SentCallBack(msgRst);
            if (flag && msgRst !== undefined) {
 ...
 *  
 */
function transferCancelSender(transferCancelSenderCb,msgObj,fileID,state){//接收端取消传输文件-----界面显示
  if(fileID){
    //dataTransferProxy.cancel(function(){},fileID);
  }
 ...
 *  
 */
function sendFileProcess(transferProcessingCb, msgObj,sendMsg) {
