...
function getLocalData(getLocalDataCb){
  imChatPack.getLocalData(getLocalDataCb);
  //getLocalDataCb(imChatPack.config);
}
exports.getLocalData = getLocalData;
...
function sendAppMsg(SentCallBack, MsgObj) {
  ...
  imChatPack.sendMSGbyUID( MsgObj.Account, MsgObj.Msg, function(recMsg){
    var intent=new base.Intent(MsgObj.App,JSON.stringify(recMsg));
...
function sendAppMsgByDevice(SentCallBack, MsgObj, wsID, flag) {
  ...
  imChatPack.sendMSGbyUID(MsgObj.Account, MsgObj.Msg,  function(recMsg) {
    var intent = new base.Intent(MsgObj.App, recMsg);
    intent.send(MsgObj.App, MsgObj.IP, function(ret) {
      if (ret.err) SentCallBack(null);
      else {
        SentCallBack(recMsg);
        if (flag) {
          recMsg['destInfo'] = {
          ...
          try {
            browserExp.wsNotify(msgToN);
          } catch(e) {
            router.wsNotify(msgToN);
          }
        }
      }
    });
...
function sendAppMsgByAccount(SentCallBack, MsgObj, wsID, flag) {
  ...
  for (var accSetItemKey in MsgObj.toAccList) {
    ...
    if (MsgObj.localUID === accSetItemKey) {
      len -= 1;
      if (countFlag === len) {
        SentCallBack(countFlag===1);
        if ((flag && msgRst !== undefined)||countFlag===1) {
          msgRst['destInfo'] = {
            'Account': MsgObj.Account,
            'UID': MsgObj.UID,
            'IP': MsgObj.IP
      ...
      if (accSetItem === undefined || !net.isIP(accSetItem.toIP)) {
        ...
        imChatPack.sendMSGbyUID(accSetItem.toAccount, MsgObj.Msg,  function(recMsg) {
          var intent = new base.Intent(MsgObj.App, recMsg);
          intent.send(MsgObj.App, accSetItem.toIP, function(ret) {
            if ((msgRst === undefined)&&(!ret.err))
              msgRst = recMsg;
            if ((++countFlag) === len) {
              SentCallBack(msgRst);
...
//function transferFileProcess(transferFileCb, msgObj, sendMsg,isLocal) {
  fileTransferClient.transferFileProcess(msgObj,function(err, rst) { //传输文件 
    ...
    if (err) { //创建文件夹------------界面显示     
      ...
      dataTransferProxy.cpFile(sendMsg.IP + ':' + msgObj.srcPath, rst.filePath, function(ret){
        ...
        if(ret.err) {
          ...
          msgObj = rst;
          msgObj['state'] = 1;
          msgObj['fileID'] =sessionID;
          transferFileCb(false, msgObj);
          dataTransferProxy.on('progress#' + sessionID, function(percentage, dir) {
...
function notifyRatio(msgObj, sendMsg, cb) {
  ...
  var sendM = {};
  var CalBakMsg = {};
  getLocalData(function(localDataJson) {
    sendM['from'] = localDataJson.account;
    sendM['uuid'] = localDataJson.UID;
...
function sendFileProcess(sendFileProcessCb, msgObj,sendMsg) {
  ...
  var to = setTimeout(function() {
    dataTransferProxy.status(sessionID, function(ret) {
      console.log(ret);
      msgObj['option'] = 0x0030;
      msgObj['state'] = 3;
      msgObj['ratio'] = 1;
...
exports.refuseFileItemTransfer = refuseFileItemTransfer;
