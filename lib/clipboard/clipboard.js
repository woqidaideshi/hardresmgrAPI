...
/* @method hasFinishDataTransfer: Judge data transfer process whether has finished.
 *  @para data: sessionId.
 *  @para callback: function to handle the reslut of judegement.
 */
exports.hasFinishDataTransfer = function(callback, sessionId) {
  dataTransProxy.status(sessionId, function(ret) {
    if (ret.ret === 'done') {
      callback(true);
    } else {
      callback(false);
...
/* @method monitorDataTransfer: Moniter data transfer status.
 *  @para data: sessionId.
...
/* @method dataTransProgress: keep consistence with the browser code, just return the parameters.
