...
exports.getIconPath = function (getIconPathCb, iconName, size){
  ...
  data.getIconPath(obj, function(retObj){
    ...
    getIconPathCb(retObj.retErr, _ret);
  });
};
...
exports.importMetaData = function(importDataBaseCb, sPath){
  console.log("Request handler 'importMetaData' was called.");
  data.importMetaData(sPath, function(retObj) {
    var _ret;
    if (retObj.ret) {
      _ret = retObj.ret;//JSON.parse(retObj.ret);
    } else {
      _ret = "notfound";
    }
    importDataBaseCb(retObj.retErr, _ret);
  });
};

exports.unZip = function(unZipCb, sfolderPath){
  console.log("Request handler 'unZip' was called.");
  data.unZip(sfolderPath, function(retObj) {
    var _ret;
    if (retObj.ret) {
      _ret = retObj.ret;//JSON.parse(retObj.ret);
    } else {
    ...
    unZipCb(retObj.retErr, _ret);
  });
}

exports.zipFolder = function(zipFolderCb, sZipFolderPath,sBackupStoreFolder){
  console.log("Request handler 'zipFolder' was called.");
  var obj = new Object();
  obj.sZipFolderPath = sZipFolderPath;
  obj.sBackupStoreFolder = sBackupStoreFolder;
  data.zipFolder(obj.sZipFolderPath, obj.sBackupStoreFolder, function(retObj) {
    var _ret;
    if (retObj.ret) {
      _ret = retObj.ret;//JSON.parse(retObj.ret);
    } else {
    ...
    zipFolderCb(retObj.retErr, _ret);
  });
}

exports.exportData = function(exportDataCb, sPath){
  console.log("Request handler 'exportData' was called.");
  data.exportData(sPath, function(retObj) {
    var _ret;
    if (retObj.ret) {
      _ret = retObj.ret;//JSON.parse(retObj.ret);
    } else {
