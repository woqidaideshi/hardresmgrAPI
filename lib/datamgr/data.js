...
exports.exportData = function(exportDataCb, sExportPath){
  console.log("Request handler 'exportData' was called.");
  data.exportData(sExportPath, function(retObj) {
    if(retObj.retErr){
      exportDataCb(retObj.retErr);
    } else {
      exportDataCb(null, retObj.ret);
    }
  });
};

exports.clearAllData = function(clearAllDataCb) {
  console.log("Request handler 'clearAllData' was called.");
  data.clearAllData(function(retObj) {
    if (retObj.retErr) {
      clearAllDataCb(retObj.retErr);
    } else {
      clearAllDataCb(null, retObj.ret);
    }
  });
};

exports.importData = function(importDataCb, sTarFilePath) {
  console.log("Request handler 'importData' was called.");
  data.importData(sTarFilePath, function(retObj) {
    if (retObj.retErr) {
      importDataCb(retObj.retErr);
    } else {
      importDataCb(null, retObj.ret);
    }
  });
};
...
exports.deleteTmpFile = function(deleteTmpFileCb, filePath) {
  fs.unlink(filePath, function(err) {
    if (err) {
      deleteTmpFileCb(err);
    } else {
      deleteTmpFileCb(null);
    }
  });
};
