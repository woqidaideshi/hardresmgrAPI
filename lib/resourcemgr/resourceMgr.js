...
ResourceMgr.prototype.applyResource = function(callback_, argsObj_) {
  ...
  self._getProxy(function(err_, proxy_) {
    ...
    proxy_.applyResource(argsObj_, function(ret_) {
      if (ret_.err){
        console.log('apply resource state error');
        return callback_(ret_.err, ret_.ret);
      } else {
        var keyM = false;
        if (ret_.ret.type[0] === 'hardResource' && ret_.ret.detail != null) {
          var de = ret_.ret.detail;
          for (var i = 0; i < de.length; i++) {
            if ((de[i]['type'][1] == 'keyboard' || de[i]['type'][1] == 'mouse') && (!keyM)) {
              keyM = true;
              proxy_.getChannel({
                type: 'mouseKey'
                ...
                self.channelMap['mouseKey']=channel.id;
                channel.on('data', function(chuck) {
                  ...
                  console.log(err);
                  return callback_('get channel error');
                });
                // setTimeout(function(){
                //   console.log('close channel----');
                //   proxy_.releaseChannel(channel.id, 'abcd',function(ret_) {
                //     console.log('release channel  '+JSON.stringify(ret_))
                //   });
                //   console.log('close channel over!!!')
                // },10000);
              });
            }
          }
          ...
          return callback_('get null resource.', ret_.ret);
        }
      }
...
ResourceMgr.prototype.releaseResource = function(callback_, argsObj_) {
  ...
  self._getProxy(function(err_, proxy_) {
    if (err_) return callback_(err_);
    proxy_.releaseChannel(argsObj_['channelId'],'abcd', function(ret_) {
      if (ret_.err) callback_(err_);
      else{
...
ResourceMgr.prototype.getChildCate = function(callback_, argsObj_) {
  var self=this;
  self._getProxy(function(err_, proxy_) {
    if (err_) return callback_(err_);
    proxy_.getCateList(argsObj_['type'], function(ret_) {
      if (ret_.err) console.log('get child cate error ');
      callback_(ret_.err, ret_.ret);
    });
  }, argsObj_);
}

ResourceMgr.prototype.iUsingItOrNot = function(callback_, channelKey) {
  var self=this;
  callback_(self.channelMap[channelKey]);
}
