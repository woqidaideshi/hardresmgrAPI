...
function getMouseKeyObj(agrsObj_){
  var agrsObj = {};
  ...
  agrsObj['desc'] = {};
  agrsObj['other'] = 0;
  agrsObj['desc']['IP'] = agrsObj_['IP'];
  return agrsObj;
}
function applyMouseKey(callback, agrsObj_) {
  var agrsObj=getMouseKeyObj(agrsObj_);
  resMgr.applyResource(callback, agrsObj);
}
exports.applyMouseKey = applyMouseKey;

function releaseMouseKey(callback, agrsObj_) {
  var agrsObj=getMouseKeyObj(agrsObj_);
  releaseResource(callback,'mouseKey', agrsObj,agrsObj_['isSelf']);
}
exports.releaseMouseKey = releaseMouseKey;
...
function releaseResource(callback, channelKey,agrsObj,isSelf) {
  if(isSelf){
    agrsObj['desc']['IP'] = undefined;
    agrsObj['channelKey']=channelKey;
    resMgr.releaseResource(callback, agrsObj);
  }else{
    resMgr.iUsingItOrNot(function(channelId){
      if(channelId===undefined) callback('you have no right to stop it.');
      else {
        agrsObj['channelId']=channelId;
        resMgr.releaseResource(callback, agrsObj);
      }
    },channelKey);
  }  
...
//   console.log("-------------->>>>>>>> " + JSON.stringify(ret_));
// },undefined,['hardResource','printer']);

// var startTime=Date.now();
// getResourceList(function(err, ret_) {
//   // console.log("-------------->>>>>>>> " + JSON.stringify(ret_));
//   fs.appendFile('/home/yff/.custard/servlog/commdaemon.log','time:'+String(Date.now()-startTime)+'\n','utf8',function(err){
//       if(err)console.log(err);
//   });
// },'192.168.160.18',['hardResource']);
...
//   agrsObj['desc'] = {};
//   agrsObj['other'] = 0;
//   agrsObj['desc']['IP'] = '192.168.160.18';
